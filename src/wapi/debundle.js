...
window.WAPI.getAllContacts = function () {
  return window.Store.Contact.map((contact) =>
...
window.WAPI.getMyContacts = function () {
  return window.Store.Contact.filter(
...
window.WAPI.getAllChatsWithNewMsg = function () {
  return window.Store.Chat.filter(window.WAPI.haveNewMsg).map((chat) =>
  ...
  return JSON.stringify(
    Store.Chat.models
      ...
      .map((chat) => {
        return {
          id: chat.id,
          indicatedNewMessages: chat.msgs.models
            .slice(Math.max(chat.msgs.length - chat.unreadCount, 0))
...
window.WAPI.getSingleProperty = function (namespace, id, property) {
  if (
    Store[namespace] &&
    Store[namespace].get(id) &&
    Object.keys(Store[namespace].get(id)).find((x) => x.includes(property))
  )
...
window.WAPI.inviteInfo = async function (link) {
  return await Store.WapQuery.groupInviteInfo(link.split('/').pop()).then((r) =>
    r.status === 200 ? WAPI.quickClean(r) : r.status
  )
}
...
window.WAPI.getGroupParticipantIDs = async function (id) {
  return (await WAPI._getGroupParticipants(id)).map(
...
WAPI.iAmAdmin = async function () {
  return (
    await Promise.all(
      Store.GroupMetadata.models.map(({ id }) => Store.GroupMetadata.find(id))
    )
  )
    .filter(
      ({ participants }) =>
        participants.iAmAdmin() || participants.iAmSuperAdmin()
    )
...
window.WAPI.getAllMessagesInChat = function (
  ...
  return (
    (clean
      ? output.map(WAPI.quickClean)
      : output.map(WAPI._serializeMessageObj)) || []
  )
}
...
window.WAPI.sendMessageWithMentions = async function (ch, body) {
  ...
  var mentionedJidList =
    body
      ...
      .filter((x) => x.length > 5)
      .map((x) =>
        Store.Contact.get(x.replace('@', '') + '@c.us')
          ? new Store.WidFactory.createUserWid(x.replace('@', ''))
          : ''
      ) || undefined
  var extend = {
    ack: 0,
...
window.WAPI.smartDeleteMessages = async function (
  chatId,
  messageArray,
  onlyLocal
) {
...
window.WAPI.deleteMessage = async function (
  chatId,
  messageArray,
  revoke = false
) {
...
window.WAPI.onAnyMessage = (callback) =>
...
window.WAPI.addOrRemoveLabels = async function (label, objectId, type) {
  ...
  var to =
    Store.Chat.get(objectId) ||
    Store.Msg.get(objectId) ||
    Store.Contact.get(objectId)
...
window.WAPI.forceUpdateLiveLocation = async function (chatId) {
  ...
  return WAPI.quickClean(
    await Store.LiveLocation.update(chatId)
  ).participants.map((l) => {
    return {
      ...l,
...
WAPI.onGlobalParicipantsChanged = function (callback) {
  const events = ['change:isAdmin', 'remove', 'add']
  ...
  const chats = Store.GroupMetadata.models
    ...
    .filter((x) => x.id.server !== 'broadcast')
    .map((group) => window.Store.Chat.get(group.id._serialized))
  ...
  Store.GroupMetadata.on('all', (eventName, groupId) =>
    chats.map((chat) => chat.groupMetadata.participants.on('all', cb))
  )
  return true
}
...
window.WAPI.onAddedToGroup = function (callback) {
  Store.Chat.on('change:previewMessage', async (event) => {
    if (
      ...
      event.previewMessage.type == 'gp2' &&
      event.previewMessage.subtype == 'add' &&
      event.previewMessage.recipients &&
      event.previewMessage.recipients
        .map((x) => x._serialized)
        .includes(Store.Me.wid._serialized)
    ) {
      const tdiff =
        (Date.now() -
          Store.Msg.get(event.previewMessage.id._serialized).t * 1000) /
        1000
      if (tdiff < 10.0) {
...
window.WAPI.joinGroupViaLink = async function (link) {
  return await Store.WapQuery.acceptGroupInvite(link.split('/').pop()).then(
    (res) => (res.status === 200 ? res.gid._serialized : res.status)
  )
  let code = link
  ...
  if (link.includes('chat.whatsapp.com')) {
    ...
    code = link
      .match(/chat.whatsapp.com\/([\w\d]*)/g)[0]
...
window.WAPI.sendImage = async function (
  imgBase64,
  chatid,
  filename,
  caption,
...
window.WAPI.setMyName = async function (newName) {
  if (!Store.Versions.default[12].BinaryProtocol)
    Store.Versions.default[12].BinaryProtocol = new Store.bp(
      Store.Me.binVersion
    )
...
window.WAPI.setGroupIcon = async function (groupId, imgData) {
  const { status } = await Store.WapQuery.sendSetPicture(
    groupId,
    imgData,
    imgData
  )
...
window.WAPI.sendVideoAsGif = async function (
  imgBase64,
  chatid,
  filename,
  caption,
...
window.WAPI.refreshBusinessProfileProducts = async function () {
  await Promise.all(
    Store.BusinessProfile.models.map(async (x) => {
      try {
 ...
 * @returns
 */
window.WAPI.sendImageWithProduct = async function (
  imgBase64,
  chatid,
  caption,
  bizNumber,
  ...
  return await Store.Catalog.findCarouselCatalog(bizNumber).then(
    async (cat) => {
      if (cat && cat[0]) {
        ...
        return Store.Chat.find(chatid).then(async (chat) => {
          var mediaBlob = window.WAPI.base64ImageToFile(
            imgBase64,
            'filename.jpg'
          )
          // var mc = new Store.MediaCollection(chat);
          // mc.processFiles([mediaBlob], chat, 1)
          return await window.WAPI.procFiles(chat, mediaBlob).then(
            async (mc) => {
              var media = mc.models[0]
 ...
 */
window.WAPI.sendVCard = async function (
  chatId,
  vcard,
  contactName,
  contactNumber
  ...
  var extend = {
    ...
    body: contactNumber
      ? vcard.replace(
          'TEL;TYPE=WORK,VOICE:',
          `TEL;TYPE=WORK,VOICE;waid=${contactNumber}:`
        )
      : vcard,
    mediaData: undefined,
    isQuotedMsgAvailable: false,
...
window.WAPI.getUseHereString = async function () {
  ...
  return window.l10n.localeStrings[window.l10n.getLocale()][0][
    window.l10n.localeStrings.en[0].findIndex(
      (x) => x.toLowerCase() === 'use here'
    )
  ]
}
...
WAPI.getChatWithNonContacts = async function () {
  return Store.Chat.models
    .map((chat) =>
      chat.contact && !chat.contact.isMyContact ? chat.contact : null
    )
    .filter((x) => x && !x.isGroup)
    .map(WAPI._serializeContactObj)
}
...
window.WAPI.getStoryStatusByTimeStamp = function () {
  return false
}
...
window.WAPI.setGroupEditToAdminsOnly = function () {
  return false
}
window.WAPI.postTextStatus = function () {
  return false
}
window.WAPI.postImageStatus = function () {
  return false
}
window.WAPI.postVideoStatus = function () {
  return false
}
window.WAPI.onRemovedFromGroup = function () {
  return false
}
window.WAPI.onContactAdded = function () {
  return false
}
window.WAPI.sendReplyWithMentions = function () {
  return false
}
window.WAPI.clearAllChats = function () {
  return false
}
window.WAPI.getCommonGroups = function () {
  return false
}
window.WAPI.setChatBackgroundColourHex = function () {
  return false
}
window.WAPI.darkMode = function () {
  return false
}
window.WAPI.onChatOpened = function () {
  return false
}
window.WAPI.onStory = function () {
  return false
}
window.WAPI.getStoryViewers = function () {
  return false
}
window.WAPI.onChatState = function () {
  return false
}
window.WAPI.getStickerDecryptable = function () {
  return false
}
window.WAPI.forceStaleMediaUpdate = function () {
  return false
}
window.WAPI.setProfilePic = function () {
  return false
}
window.WAPI.setGroupDescription = function () {
  return false
}
window.WAPI.setGroupTitle = function () {
  return false
}
window.WAPI.tagEveryone = function () {
  return false
}
 ...
 * Patches
 */
window.WAPI.sendGiphyAsSticker = function () {
  return false
}
window.WAPI.getBlockedIds = function () {
  return false
}
